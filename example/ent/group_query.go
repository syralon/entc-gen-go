...
func (_q *GroupQuery) QueryGroupUsers() *UserQuery {
	...
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		...
		step := sqlgraph.NewStep(
			...
			sqlgraph.Edge(sqlgraph.M2M, true, group.GroupUsersTable, group.GroupUsersPrimaryKey...),
		)
...
func (_q *GroupQuery) Clone() *GroupQuery {
	...
	return &GroupQuery{
		...
		withGroupUsers: _q.withGroupUsers.Clone(),
...
func (_q *GroupQuery) WithGroupUsers(opts ...func(*UserQuery)) *GroupQuery {
	query := (&UserClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withGroupUsers = query
	return _q
}
...
func (_q *GroupQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Group, error) {
	...
	if query := _q.withGroupUsers; query != nil {
		if err := _q.loadGroupUsers(ctx, query, nodes,
			func(n *Group) { n.Edges.GroupUsers = []*User{} },
			func(n *Group, e *User) { n.Edges.GroupUsers = append(n.Edges.GroupUsers, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *GroupQuery) loadGroupUsers(ctx context.Context, query *UserQuery, nodes []*Group, init func(*Group), assign func(*Group, *User)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(group.GroupUsersTable)
		s.Join(joinT).On(s.C(user.FieldID), joinT.C(group.GroupUsersPrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(group.GroupUsersPrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(group.GroupUsersPrimaryKey[1]))
	...
	for _, n := range neighbors {
		nodes, ok := nids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected "group_users" node returned %v`, n.ID)
		}
		for kn := range nodes {
			assign(kn, n)
		}
