...
func (_c *GroupCreate) AddGroupUserIDs(ids ...int) *GroupCreate {
	_c.mutation.AddGroupUserIDs(ids...)
	return _c
}

// AddGroupUsers adds the "group_users" edges to the User entity.
func (_c *GroupCreate) AddGroupUsers(v ...*User) *GroupCreate {
	ids := make([]int, len(v))
	for i := range v {
		ids[i] = v[i].ID
	}
	return _c.AddGroupUserIDs(ids...)
}
...
func (_c *GroupCreate) createSpec() (*Group, *sqlgraph.CreateSpec) {
	...
	if nodes := _c.mutation.GroupUsersIDs(); len(nodes) > 0 {
